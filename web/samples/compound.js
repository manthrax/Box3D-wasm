...
export function createCompoundSamples( { BodyType } )
{
	return [
		...
		},
		{
			...
			description:
				"A browser-first approximation of the native Village sample: a huge mixed compound made from thousands of repeated ground hulls plus scattered spheres and capsules. The native sample layers on character movement, ray casts, and mesh materials; this version keeps the large-scene collision focus and adds a few dynamic probes so the world reads immediately.",
			create( ctx )
			{
				const gridCount = 64;
				const worldScale = 4;
				...
				return {
					reset()
					{
						...
						for ( let i = 0; i < gridCount; i += 1 )
						{
							for ( let j = 0; j < gridCount; j += 1 )
							{
								...
								if ( ( i & 1 ) === 1 && ( j & 1 ) === 1 )
								{
									...
									const p1 = {
										x: localPosition.x + offset1.x,
										y: localPosition.y + offset1.y,
										z: localPosition.z + offset1.z,
									};
									const p2 = {
										x: localPosition.x + offset2.x,
										y: localPosition.y + offset2.y,
										z: localPosition.z + offset2.z,
									};
						...
						for ( let i = 0; i < 5; i += 1 )
						{
							probeHandles.push(
								ctx.physics.createSphereBody( {
									type: BodyType.dynamic,
									position: { x: -60 + 30 * i, y: 38 + 4 * i, z: -55 + 24 * i },
									radius: 1.2,
									color: 0xd67c42,
								} )
							);
					...
					getStatusLines()
					{
						const firstProbe = ctx.physics.getBodyTransform( probeHandles[0] );
						return [
							`compound capsules / hulls / spheres: ${capsuleCount} / ${hullCount} / ${sphereCount}`,
							`probe height: ${firstProbe.position.y.toFixed( 2 )}`,
							`note: browser port skips native character + query debug overlay`,
							`bodies: ${ctx.physics.getBodyCount()}`,
						];
					},
				};
